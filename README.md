...
So i could have continuous builds of my configurations built automatically by github for every new Marlin release for free.   
Any time i want to upgrade i just go to the repo and grab a download.  
I open-sourced it and made it reusable for everyone to create and share builds for their printers.  
While it's convenient for anyone mantaining their own configurations, it's also a great way for users that don't want to mess on their own
with the setup and compilation complexity to be able to just download upgrades.  
If this is actually proved to be useful and take off, we could end with a nice decentralized "network" of all different kinds of configurations for everyone to share and use.

## Builds using Marlin auto build
